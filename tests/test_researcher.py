...
class TestResearcher:
    ...
    def test_handles_research_states_correctly(self, empty_graph_state):
        """Test that researcher correctly handles research states by research step index."""
        # Arrange
    ...
    def test_retrieves_messages_for_specific_research_index(self, empty_graph_state):
    ...
    def test_adds_new_researcher_state_for_new_step(self, empty_graph_state):
    ...
    def test_uses_existing_research_state_for_retry(self, empty_graph_state):
        ...
        state["researcher_states"] = {
            1: {"messages": [Mock()], "step_index": 1, "result": None}
        }
        state["agent_messages"] = [
        ...
        with patch('multi_agent_system.compiled_researcher_graph') as mock_graph:
            mock_graph.invoke.return_value = {
                "messages": [Mock(), Mock()],  # Additional message added
                "step_index": 1,
    ...
    def test_dynamically_builds_research_results(self, empty_graph_state):
    ...
    def test_overwrites_research_results_on_retry(self, empty_graph_state):
    ...
    def test_research_results_not_pre_allocated(self, empty_graph_state):
    ...
    def test_sends_completion_message_to_orchestrator(self, empty_graph_state):
        """Test that researcher sends correct message to orchestrator that research is complete."""
        # Arrange
        state = empty_graph_state.copy()
        state["current_research_index"] = 0
        state["research_steps"] = ["Research step 1"]
        state["agent_messages"] = [
            {"sender": "orchestrator", "receiver": "researcher", "type": "instruction", "content": "test", "step_id": 0}
        ]
    ...
    def test_handles_all_research_steps_completed(self, empty_graph_state):
        ...
        state = empty_graph_state.copy()
        state["current_research_index"] = 2
        state["research_steps"] = ["Step 1", "Step 2"]  # Only 2 steps, but index is 2
        
        # Act
        result = researcher_node(state)
        
        # Assert: No new researcher state is created when all steps complete
        # This would be handled by the orchestrator logic, but we can test that the researcher
        # doesn't crash when given an invalid index
        assert "researcher_states" in result

    def test_researcher_with_multiple_tool_calls(self, empty_graph_state):
        """Test researcher with multiple tool calls in a single research step."""
        # Arrange
        state = empty_graph_state.copy()
        state["current_research_index"] = 0
        ...
        with patch('multi_agent_system.compiled_researcher_graph') as mock_graph:
            mock_graph.invoke.return_value = {
                "messages": [Mock(), Mock(), Mock()],  # Multiple messages from tool calls
                "step_index": 0,
    ...
    def test_researcher_state_preservation_across_retries(self, empty_graph_state):
        """Test that researcher state is preserved correctly across retries."""
        # Arrange
        state = empty_graph_state.copy()
        state["current_research_index"] = 1
        state["research_steps"] = ["Step 1", "Step 2"]
        state["researcher_states"] = {
            1: {
                "messages": [Mock(), Mock()],  # Previous messages
                "step_index": 1,
                "result": None
            }
        }
        ...
        with patch('multi_agent_system.compiled_researcher_graph') as mock_graph:
            mock_graph.invoke.return_value = {
                "messages": [Mock(), Mock(), Mock()],  # Additional message
                "step_index": 1,
            ...
            assert result["researcher_states"][1]["step_index"] == 1

    def test_researcher_message_content_includes_research_step(self, empty_graph_state):
